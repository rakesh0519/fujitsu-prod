...
resource "azurerm_storage_account" "storeacc" {
  name                      = substr(format("str%s%s", lower(replace(var.storage_account_name, "/[[:^alnum:]]/", "")), random_string.unique.result), 0, 24)
    ...
    content {
      ...
      actions {
        base_blob {
          ...
          # Commenting out archive tier to avoid the error
