...
output "key_vault_private_endpoint" {
  ...
  value       = var.enable_private_endpoint ? element(concat(azurerm_private_endpoint.pep2.*.id, [""]), 0) : null
}

output "key_vault_private_dns_zone_domain" {
  description = "DNS zone name for Key Vault Private endpoints dns name records"
  value       = var.existing_private_dns_zone == null && var.enable_private_endpoint ? element(concat(azurerm_private_dns_zone.dnszone2.*.name, [""]), 0) : var.existing_private_dns_zone
}

output "key_vault_private_endpoint_ip_addresses" {
  description = "Key Vault private endpoint IPv4 Addresses"
  value       = var.enable_private_endpoint ? flatten(azurerm_private_endpoint.pep2.0.custom_dns_configs.*.ip_addresses) : null
}

output "key_vault_private_endpoint_fqdn" {
  description = "Key Vault private endpoint FQDN Addresses"
  value       = var.enable_private_endpoint ? flatten(azurerm_private_endpoint.pep2.0.custom_dns_configs.*.fqdn) : null
}